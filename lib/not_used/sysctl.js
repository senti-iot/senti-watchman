...
$ systemctl is-active --quiet senti-mqtt-client.service
...
const isActive = async (serviceName) => {
	result = await run("is-active", serviceName)
	return result.stdout == 'active\n' ? true : false
}

isActive('senti-mqtt-client.service').then(output => console.log(output))
// console.log(isActive('senti-mqtt-client.service'))
